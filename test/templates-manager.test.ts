...
const mockedLoadFile = vi.mocked(loadFile)
...
describe('mergeTemplates', () => {
  it('returns wrapped templates string from a single file', async () => {
    mockedLoadFile.mockResolvedValue('<t t-name="Comp"><div/></t>')
  ...
  it('skips failed files and logs error', async () => {
    ...
    mockedLoadFile.mockRejectedValue(new Error('404'))
  ...
  it('calls loadFile once per path', async () => {
    mockedLoadFile.mockResolvedValue('<t/>')
    await mergeTemplates(['/a.xml', '/b.xml', '/c.xml'])
    expect(mockedLoadFile).toHaveBeenCalledTimes(3)
  })
})