...
describe('processRoutes', () => {
  it('resolves a matching route for /', async () => {
    ...
    const result = await processRoutes(routes as any)
  ...
  it('throws when no route matches', async () => {
    setPath('/not-found')
    const routes = [makeRoute('/')]
    await expect(processRoutes(routes as any)).rejects.toThrow('No route found')
  })

  it('does not duplicate SSG paths on repeated calls', async () => {
    setPath('/')
    const route = makeRoute('/')
    await processRoutes([route] as any)
    setPath('/index.html')
    await processRoutes([route] as any)

    const indexHtmlCount = route.path.filter((candidate) => candidate === '/index.html').length
