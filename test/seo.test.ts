...
describe('SEO', () => {
  ...
  describe('generateSitemap', () => {
    ...
    it('should include changefreq when provided', () => {
      const sitemap = generateSitemap([{ url: '/', changefreq: 'daily' }], { baseUrl: 'https://example.com' })
      expect(sitemap).toContain('<changefreq>daily</changefreq>')
    })

    it('should include lastmod when provided', () => {
      const sitemap = generateSitemap([{ url: '/', lastmod: '2024-01-15' }], { baseUrl: 'https://example.com' })
      expect(sitemap).toContain('<lastmod>2024-01-15</lastmod>')
    })

    it('should handle absolute URLs', () => {
      const sitemap = generateSitemap([{ url: 'https://other.com/page' }], { baseUrl: 'https://example.com' })
      expect(sitemap).toContain('<loc>https://other.com/page</loc>')
    })

    it('should normalize baseUrl trailing slash', () => {
      const sitemap = generateSitemap([{ url: '/page' }], { baseUrl: 'https://example.com/' })
  ...
  describe('jsonLd', () => {
    it('should generate JSON-LD with default context', () => {
      ...
      const parsed = JSON.parse(schema) as Record<string, string>
  ...
  describe('createCanonicalUrl', () => {
    ...
    it('should remove query parameters when specified', () => {
      const url = createCanonicalUrl('https://example.com', '/page?id=123', { removeQueryParams: true })
      expect(url).toBe('https://example.com/page')
    })

    it('should filter allowed query parameters', () => {
      const url = createCanonicalUrl('https://example.com', '/page?id=123&utm_source=email', { allowedParams: ['id'] })
      expect(url).toBe('https://example.com/page?id=123')
    })
  ...
  describe('validateSitemap', () => {
    it('should validate correct entries', () => {
      const result = validateSitemap([{ url: '/', priority: 0.5, changefreq: 'daily' }])
  ...
  describe('getPriorityByDepth', () => {
    ...
    it('should decrease priority by depth', () => {
      expect(getPriorityByDepth('/products')).toBeGreaterThan(getPriorityByDepth('/products/electronics/phones'))
    })
